...
http.get('https://i.postimg.cc/P5C9fybp/Logo-et-nom.png', (res) => {
  res.pipe(new PNG())
    .on('parsed', function() {
      // Find the most common color
      // Just check the top-left pixel (0,0) as background
      let bgR = this.data[0], bgG = this.data[1], bgB = this.data[2], bgA = this.data[3];
      console.log('Background color:', bgR, bgG, bgB, bgA);
